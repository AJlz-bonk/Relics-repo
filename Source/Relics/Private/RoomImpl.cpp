...
void RoomImpl::drawLine(const bool isVert, std::pair<int, int>* wall, std::pair<int, int>* next, TwoDArray& grid) const
{
    if (isVert)
    {
        ...
        for (int i = start; i <= end; i++)
        {
            ...
            {
                grid.set(i + row, wall->second + col, static_cast<char>('0' + id));
            } else
            {
    ...
    else
    {
        ...
        for (int j = start; j <= end; j++)
        {
            ...
            {
                grid.set(wall->first + row, j + col, static_cast<char>('0' + id));
            } else
            {
...
void RoomImpl::reshapeL(RandomGenerator& rg)
{
    ...
    unsigned int c = col + rg.getRandom(min_size, static_cast<int>(width - 1) - min_size);
    unsigned int r = row + rg.getRandom(min_size, static_cast<int>(height - 1) - min_size);
...
void RoomImpl::reshapeU(RandomGenerator& rg)
{
    ...
    while (min_interior > static_cast<int>(wall % 2 == 0 ? height : width) - 1 - min_size - min_size)
    {
        if (--min_interior <= 2)
        {
            return;
    ...
    if (wall == 0)
    {
        unsigned int c = rg.getRandom(0, static_cast<int>(width) - 1- min_size - min_size) + col + min_size;
    ...
    {
        unsigned int r = rg.getRandom(0, static_cast<int>(height) - 1 -min_size - min_size) + row + min_size;
...
void RoomImpl::addDoorToWall(RandomGenerator& rg, bool isVert, std::pair<int, int> next, std::pair<int, int>* wall, bool hasDoor)
{
    int threshold = 4;
